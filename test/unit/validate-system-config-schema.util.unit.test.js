    ...
    const invalidValueCases = [
      {
        ...
        mutate: (cfg) => { cfg.acceptedSystemArtifacts = []; },
      ...
      },
      {
        ...
        mutate: (cfg) => { cfg.connectionInsertionTitleRegex = '([a-z'; },
        expectedError: 'Must be a valid regular expression string',
      },
      {
        description: 'empty array inside artifactCategoryMap category',
        mutate: (cfg) => { cfg.classificationGuidelines.artifactCategoryMap['📕 Architecture'] = []; },
        expectedError: 'Category "📕 Architecture" must contain at least one artifact code',
      },
      {
        description: 'non-string inside artifactCategoryMap category',
        mutate: (cfg) => { cfg.classificationGuidelines.artifactCategoryMap['📕 Architecture'] = [123]; },
        expectedError: 'Artifact code must be a non-empty string',
      },
    ];
