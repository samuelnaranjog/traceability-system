...
if(!isConfig){
    configPath = createConfigFile(configName);
    ...
    await setUpPropertiesOfConfig("projectPrefix", ENGINE_TYPE, configPath)
}
else{
...
const prefix = accessPropertiesOfConfig('projectPrefix', ENGINE_TYPE, configPath); // TEMP: set right var

// Implements: ADR-012 - Extract data for the git config 
// TODO: Implement a helper to do so extracting the data and setting it up when it doesnt exist
...
if (values.symlink && !values.code ) {
    ...
    // Implements: ADR-012
    wormHoleStart = accessGitConfigSymValue(ENGINE_TYPE, 'markdownEditorFolderPath')
