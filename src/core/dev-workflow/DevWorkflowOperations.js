 ...
 */
export default class DevWorkflowOperations {
  /**
...
static async setUpPropertiesOfConfig(property, engineType, absolutePath) {
  let configData;
  let configJSON;

  //  Read & parse json file safely
  configJSON = parseFileAndCatch(absolutePath)
  ...
  if (jsonProp === undefined || jsonProp === null || jsonProp === "") {
    ...
    try {
      ...
      // this.saveConfigSettings(systemConfigName) not useful to commit that with new system;
...
static mergeOperation(cwd = process.cwd(), branch){
 ...
 export const findWorkTreePath = (...args) => DevWorkflowOperations.findWorktreePath(...args);
