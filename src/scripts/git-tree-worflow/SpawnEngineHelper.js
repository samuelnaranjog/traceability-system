import {isParentPrefix, findMainWorkTreePath, findMainWorkTreePathParent, createPrefixFolder, moveTreesToParent }  from "./GitWorflowOperations"

/**
 * @param {string} prefix
 * @returns {string} The path to the prefix directory
 */
export function folderSetUp(prefix){
     /**
        ...
        */
        ...
        const isParentPrex = isParentPrefix(prefix); 

        // Handle the Bool cases
        if (isParentPrex == false) {
            // Check if the main tree has a prex folder
            const mainTreePath = findMainWorkTreePath();
            const isMainPrex = isParentPrefix(prefix, mainTreePath);
    
            // Check all the trees and move them to the to the prex folder if they are not in there
            if (isMainPrex == true) {
                
                prexDirPath = findMainWorkTreePathParent();
                moveTreesToParent(prefix, prexDirPath)
                
            }
            else {
                // Create the prefix folder relative to the main tree and move trees there
                prexDirPath = createPrefixFolder(prefix);
                moveTreesToParent(prefix, prexDirPath);
            }
        }
